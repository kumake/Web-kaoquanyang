			...
			<tr>
				...
				<div id="neiye_main">
								<!--内容开始 -->
										...
										if total = 0 then 
										Dim Tarray:Tarray = UICon.QueryData("select count(*) as total from user_yhxx where "&strwhere&"")
										...
										'Dim sql:sql = UICon.QueryPageByNum("categoryID,id,title,posttime","user_yhxxs", ""&strwhere&"", "ID", true,current_page,pagesize)
										'这种方式为根据IndexID排序，IndexID值越大越靠前
										Dim sql:sql = UICon.QueryPageByNotIn("*","user_yhxx", ""&strwhere&"", "ID", "indexID desc,ID",false,current_page,pagesize)
										'response.Write sql
										'response.End()
										...
										<span style="font-family:'宋体'">·</span><a href="yhxx_in.asp?categoryid=<%=rsn("categoryID")%>&amp;itemid=<%=rsn("id")%>" class="y_0"><%=rsn("title")%></a> </td>
										</tr>
										<tr>
